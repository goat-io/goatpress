  ...
  # Determine the PHP-FPM runtime environment
   ...
   # Opcache settings
    if [[ -z $PHP_OPCACHE_ENABLE ]]; then 
      PHP_OPCACHE_ENABLE=1 && echo "${PHP_OPCACHE_ENABLE}"; 
    fi
   ...
   # Set the listening port
    if [[ -z $PHP_FPM_PORT ]]; then 
      echo "PHP-FPM port not set. Default to 9000..." && export PHP_FPM_PORT=9000; 
    else 
      echo "OK, PHP-FPM port is set to $PHP_FPM_PORT"; 
    fi
   # Set the document root. This is usually the same as your NGINX docroot
   if [[ -z $APP_DOCROOT ]]; then export APP_DOCROOT=/usr/share/nginx/html && mkdir -p "${APP_DOCROOT}"; fi
...

{
      ...
      echo 'session.save_path="tcp://{{NGINX_REDIS_UPSTREAM}}?weight=1&timeout=2.5&database=3"'
} | tee /etc/php8/conf.d/zz-redis-setting.ini

  find /etc/php8 -maxdepth 3 -type f -exec sed -i -e 's|{{NGINX_REDIS_UPSTREAM}}|'"${NGINX_REDIS_UPSTREAM}"'|g' {} \;
...
function run() {
  ...
  if [[ -z $WP_REDIS_DISABLED ]]; then 
    echo "Redis is not present so we will not activate it"; 
  else 
    echo "Configuring PHP-REDIS"; 
    redis; 
  fi
