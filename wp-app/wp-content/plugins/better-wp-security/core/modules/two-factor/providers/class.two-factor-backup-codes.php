...
class Two_Factor_Backup_Codes extends Two_Factor_Provider implements ITSEC_Two_Factor_Provider_On_Boardable, ITSEC_Two_Factor_Provider_CLI_Configurable {
	...
	public function get_label() {
		return _x( 'Backup Authentication Codes', 'Provider Label', 'better-wp-security' );
	}
	...
	public function description() {
		echo '<p class="description">' . __( 'Provide a set of one-time use codes that can be used to login in the event the primary two-factor method is lost. Note: these codes are intended to be stored in a secure location.', 'better-wp-security' ) . '</p>';
	}
	...
	public function get_on_board_label() {
		return esc_html__( 'Backup Codes', 'better-wp-security' );
	}
	...
	public function get_on_board_description() {
		return esc_html__( 'A list of one-time codes you can use if you lose access to your device.', 'better-wp-security' );
	}
	...
	public function handle_ajax_on_board( WP_User $user, array $data ) {
		if ( $data['itsec_method'] === 'generate-backup-codes' ) {
			...
			wp_send_json_success( array(
				'message'    => esc_html__( 'Codes Generated', 'better-wp-security' ),
				'codes'      => $new,
